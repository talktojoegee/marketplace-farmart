...
class OrderSupportServiceProvider extends ServiceProvider
{
    ...
    public function processPostCheckoutOrder(
        ...
        $pMethod = $request->payment_method ?? null;
        $refCode = substr(sha1(time()), 29,40);
        $pluckedOrderIds = $orders->pluck('id')->toArray();
        $productOrders = OrderProduct::getOrderProductDetails($pluckedOrderIds);
        switch ($pMethod){
            case 'savings':
                $this->handlePaymentByCoopOption($refCode, $memberId, $totalAmount, now(), $productOrders, 1);
                break;
            case 'loan':
                $this->handlePaymentByCoopOption($refCode, $memberId, $totalAmount, now(), $productOrders, 2);
                break;
            case 'paystack':
                $orders  = [];
                if(count($productOrders) > 0) {
                    foreach ($productOrders as $detail) {
                        $order = Order::getOrderById($detail->order_id);
                        $store = Store::getStoreById($order->store_id);
                        $data = [
                            "store_id" => $order->store_id,
                            "vendor_name" => $store->name ?? 'N/A',
                            "Product_code" => $detail->code ?? 'N/A',
                ...
                $form = $this->buildForm($memberId, $refCode, $orders);
                $this->chargeCard($totalAmount, $form, $email, $refCode, now(), $products, $request, $token, $mpSessionData);
                break;
        }
    ...
    public function handlePaymentByCoopOption($refCode, $memberId, $amount, $orderDate, $productOrders, $pMethod){
        $orders  = [];
        if(count($productOrders) > 0) {
            foreach ($productOrders as $detail) {
                $order = Order::getOrderById($detail->order_id);
        ...
        try{
            ...
            $savingsApiResponse = $this->postPaymentNotification($refCode, $memberId,$amount, $orderDate, $pMethod);
    ...
    public function buildForm($memberId, $refCode, $orders){
        $form = [
            "uid" => $memberId ?? 'TEST',
            "TransID" => $refCode,
            "OrderID" => 56,//$refCode,
    ...
    public function chargeCard($amount, $data, $email, $orderCode, $orderDate, $products, $request, $token, $mpSessionData){
        try{
            $paystack = new Paystack(env('PAYSTACK_SECRET_KEY'));
            $builder = new Paystack\MetadataBuilder();
            $builder->withTransaction(3);
            $builder->withOrder($data);
            //return dd($products, $request, $token, $mpSessionData);
            //$builder->withProducts($products);
            //$builder->withRequest($request);
            //$builder->withToken($token);
            //$builder->withMpsessiondata($mpSessionData);
