...
class PublicCheckoutController
{
    ...
    public function postCheckout(
        ...
        //return dd('selected payment method '.$paymentMethod);
    ...
    public function processOnlinePayment(Request $request){
        ...
        if ($tranx->data->status  === 'success') {
            try {
                ...
                $products = $tranx->data->metadata->products;
                $req  = $tranx->data->metadata->request;
                $token  = $tranx->data->metadata->token;
                $mpSessionData  = $tranx->data->metadata->mpsessiondata;
               ...
               /*$req = $this->sendAPIRequest($extUrl, json_encode($form));
                do_action('ecommerce_before_processing_payment', $products, $req, $token, $mpSessionData);
                $paymentData = $this->processPaymentMethodPostCheckout($request, (float)$amount);
