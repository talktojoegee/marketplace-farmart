            ...
            add_filter(FILTER_ECOMMERCE_PROCESS_PAYMENT, function (array $data, Request $request) {
                ...
                switch ($request->input('payment_method')) {
                    ...
                    case PaymentMethodEnum::BANK_TRANSFER:
                        ...
                        break;
                    case PaymentMethodEnum::SAVINGS:
                        if ($minimumOrderAmount > Cart::instance('cart')->rawSubTotal()) {
                            $data['error'] = true;
                            $data['message'] = __('Insufficient savings balance', ['amount' => format_price($minimumOrderAmount), 'more' => format_price($minimumOrderAmount - Cart::instance('cart')->rawSubTotal())]);
                            break;
                        }
                        $data['charge_id'] = $this->app->make(CodPaymentService::class)->execute($paymentData);
                    ...
                    case PaymentMethodEnum::LOAN:
                        if ($minimumOrderAmount > Cart::instance('cart')->rawSubTotal()) {
                            $data['error'] = true;
                            $data['message'] = __('Insufficient loan balance', ['amount' => format_price($minimumOrderAmount), 'more' => format_price($minimumOrderAmount - Cart::instance('cart')->rawSubTotal())]);
                            break;
                        }
                        $data['charge_id'] = $this->app->make(CodPaymentService::class)->execute($paymentData);

                    case PaymentMethodEnum::PAYSTACK:
                        if ($minimumOrderAmount > Cart::instance('cart')->rawSubTotal()) {
                            $data['error'] = true;
                            $data['message'] = __('Something went wrong. Try again later', ['amount' => format_price($minimumOrderAmount), 'more' => format_price($minimumOrderAmount - Cart::instance('cart')->rawSubTotal())]);
                            break;
                        }
                        $data['charge_id'] = $this->app->make(CodPaymentService::class)->execute($paymentData);
                    default:
